...
public class TEC {
	...
	*/
	...
	public static void addAspect(UUID playerUUID, ScanResult scan, double chanceToIncrease,
			double increaseMult) {
	...
	private static void addAspect(EntityPlayer player, ScanResult scan, double chanceToIncrease,
			double increaseMult) {
		...
		if (aspects != null) {
			...
			for (Aspect aspect : aspects.getAspects()) {
				if (pk.hasDiscoveredParentAspects(player.getCommandSenderName(), aspect)) {
					...
					if (chanceToIncrease > 0d &&
							player.getEntityWorld().rand.nextDouble() < chanceToIncrease)
