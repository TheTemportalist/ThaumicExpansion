...
public class OperationDecomposer implements IOperation {
	...
	public OperationDecomposer(int decomposerTier, TEThaumicAnalyzer tile) {
		...
		this.refreshOperationVars(tile);
	}

	private void refreshOperationVars(TEThaumicAnalyzer tile) {
		if (tile.getInput() != null) {
			this.outputAspects = this.filterAspects(
					ThaumcraftApiHelper.getObjectAspects(tile.getInput()),
					this.machineTier, tile.getAugments()
			);
	...
	public void updateAugments(IOperator operator, Set<EnumAugmentTA> augments) {
		this.refreshOperationVars((TEThaumicAnalyzer) operator);
