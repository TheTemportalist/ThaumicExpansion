package com.temportalist.thaumicexp.client

import com.temportalist.thaumicexp.common.ProxyCommon
import com.temportalist.thaumicexp.common.container.ContainerAnalyzer
import com.temportalist.thaumicexp.common.init.TECBlocks
import com.temportalist.thaumicexp.common.tile.TEAnalyzer
import cpw.mods.fml.client.registry.ClientRegistry
import net.minecraft.entity.player.EntityPlayer
import net.minecraft.item.Item
import net.minecraft.tileentity.TileEntity
import net.minecraft.world.World
import net.minecraftforge.client.MinecraftForgeClient

/**
 *
 *
 * @author TheTemportalist
 */
class ProxyClient extends ProxyCommon {

	override def registerRender(): Unit = {
		ClientRegistry.bindTileEntitySpecialRenderer(classOf[TEAnalyzer], RenderAnalyzer)
		MinecraftForgeClient.registerItemRenderer(
			Item.getItemFromBlock(TECBlocks.analyzer), RenderAnalyzer)
	}

	override def getClientElement(ID: Int, player: EntityPlayer, world: World, x: Int, y: Int,
			z: Int, tileEntity: TileEntity): AnyRef = {
		if (tileEntity != null) tileEntity match {
			case te: TEAnalyzer =>
				return new GuiAnalyzer(new ContainerAnalyzer(te, player))
			case _ =>
		}
		null
	}

}
