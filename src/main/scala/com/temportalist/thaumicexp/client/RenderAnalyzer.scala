...
object RenderAnalyzer extends TERenderer(null) with IItemRenderer {
	...
	override def renderItem(iType: ItemRenderType, item: ItemStack, data: AnyRef*): Unit = {
		...
		else if (iType == ItemRenderType.INVENTORY) GL11.glTranslated(0, -0.1, 0)
		...
		tileEntity match {
			case tile: TEAnalyzer =>
				...
				*/
				...
				this.renderBlock(tile)
				this.renderScanner(tile.getWorldObj)
				GL11.glPopMatrix()
	...
	def renderInput(tile: TEAnalyzer, ticks: Float): Unit = {
		if (tile.getInput() != null) {
			...
			stack.stackSize = 1
			val entity: EntityItem = new EntityItem(tile.getWorldObj,
				0.0D, 0.0D, 0.0D, stack)
			entity.hoverStart = MathHelper.sin(ticks / 14.0F) * 0.2F + 0.2F
