...
public class TEThaumicAnalyzer extends TileMachineBase implements ISidedInventory,
	...
	public void updateEntity() {
		...
		this.checkOutput();
		//if (this.getProgress() > 0) FMLLog.info("[TEC]" + this.getProgress());
	}

	private void analyzerUpdate() {
		if (this.currentOperation == null) {
			this.currentOperation = new OperationAnalyzer(
					this.getAugmentedTime(2), // 10
		...
		if (this.canOperate() && this.getInput() != null) {
			if (!this.currentOperation.isRunning()) {
				...
				if (!this.currentOperation.canRun(this, this)) {
					...
					//		this.currentOperation.getTicks());
	...
	public Set<EnumAugmentTA> getAugments() {
		return this.augmentList.keySet();
	}
	...
	public void setPlacedBy(EntityPlayer player) {
		...
		if (playerTracker != null) {
			...
			FMLLog.info("[TEC] Null player tracker");
	}
	...
	public int getInventoryStackLimit() {
		return this.augmentList.containsKey(EnumAugmentTA.DECOMPOSER) ?
				super.getInventoryStackLimit(): 1;
	}
	...
	public void syncOperation(boolean justTicks) {
		PacketCoFHBase packet = new PacketTileInfo(this).addString("SYNC_OP").addBool(justTicks);
		if (justTicks)
			packet.addInt(this.currentOperation != null ? this.currentOperation.getTicks() : 0);
		else {
	...
	public void handleTileInfoPacket(PacketCoFHBase packet, boolean isServer, EntityPlayer player) {
		...
		if (type.equals("ENERGY")) {
			...
			Aspect aspect = this.aspects.getAspectsSorted()[index];
			if (aspect == null) {
				FMLLog.info("[TEC] Error, null aspect at index " + index + " in tile aspects");
				return;
			}
		...
		else if (type.equals("SYNC_OP")) {
			boolean justTicks = packet.getBool();
			if (justTicks) {
				int ticks = packet.getInt();
				if (this.currentOperation != null)
					this.currentOperation.setTicks(ticks);
			}
			else
				try {
					this.readFromNBT(packet.readNBT());
				} catch (Exception e) {e.printStackTrace();}
		}
	}
	...
	public void writeToNBT(NBTTagCompound tagCom) {
		...
		*/
		...
		tagCom.setInteger("energyCapacity", this.energyStorage.getMaxEnergyStored());
	...
	public void readFromNBT(NBTTagCompound tagCom) {
		...
		*/
		...
		this.energyStorage.setCapacity(tagCom.getInteger("energyCapacity"));
